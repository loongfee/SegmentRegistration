...
bool AffineLineRegistration::initialization()
{
	...
	vector<LineStruct> srcLineList;
	vector<LineStruct> refLineList;
	FILE *pf = fopen("line_intersections.txt", "w+");
	for (int i = 0; i < m_nX; i++)
	{
		intersection(m_source_lines_polar[i], m_source_lines_polar, srcLineList, 2.0);
		fprintf(pf, "%d\n", srcLineList.back().intersections.size());
	}
	fprintf(pf, "\n\n");
	for (int i = 0; i < m_nY; i++)
	{
		intersection(m_reference_lines_polar[i], m_reference_lines_polar, refLineList, 2.0);
		fprintf(pf, "%d\n", refLineList.back().intersections.size());
	}
	fclose(pf);
	Eigen::MatrixXd distMatrix;
	pairwise_distance(srcLineList, refLineList, distMatrix);

	int n1 = (int)srcLineList.size();
	int n2 = (int)refLineList.size();
	int nCount = 0;
	for (int i = 0;i < n1;++i)
	{
		if (nCount > 6)
		...
		for (int j = 0;j < n2;++j)
		{
			if (distMatrix(i,j) > 0.0001)
			{
				m_matched_ref_ls.push_back(srcLineList[i].line);
				m_matched_src_ls.push_back(refLineList[j].line);
				nCount++;
			}
		}
	}

	cv::Mat img_source = cv::imread(m_sourceImageFile);
	cv::Mat img_reference = cv::imread(m_referenceImageFile);
	cv::Mat img_outImage;
	drawLineMatches(img_source, m_matched_src_ls, img_reference, m_matched_ref_ls,
		img_outImage, cv::Scalar(0, 0, 255), cv::Scalar::all(-1),
		vector<vector<char> >(), cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS, 2);
	cv::imwrite("match.png", img_outImage);

	m_srcMaxSize = 0.0;
	for (int i = 0; i < m_nX; i++)
	{
		double m = max(fabs(m_source_lines_polar[i].pt1.x), fabs(m_source_lines_polar[i].pt1.y));
		double n = max(fabs(m_source_lines_polar[i].pt2.x), fabs(m_source_lines_polar[i].pt2.y));
		m = max(m, n);
		m_srcMaxSize = (m_srcMaxSize < m)?m:m_srcMaxSize;
	}

	m_refMaxSize = 0.0;
	for (int j = 0; j < m_nY; j++)
	{
		double m = max(fabs(m_reference_lines_polar[j].pt1.x), fabs(m_reference_lines_polar[j].pt1.y));
		double n = max(fabs(m_reference_lines_polar[j].pt2.x), fabs(m_reference_lines_polar[j].pt2.y));
		m = max(m, n);
		m_refMaxSize = (m_refMaxSize < m)?m:m_refMaxSize;
	}

	m_srcIntersections.clear();
	m_refIntersections.clear();
	double maxSize = 1.0*m_srcMaxSize;
	for (int i = 0; i < m_nX-1; i++)
	{
		for (int j = i+1; j < m_nX; j++)
		{
			intersection(m_source_lines_polar[i], m_source_lines_polar[j], m_srcIntersections, maxSize);
		}
	}
	maxSize = 1.0*m_srcMaxSize;
	for (int i = 0; i < m_nY-1; i++)
	{
		for (int j = i+1; j < m_nY; j++)
		{
			intersection(m_reference_lines_polar[i], m_reference_lines_polar[j], m_refIntersections, maxSize);
		}
	}
	m_nX = (int)m_srcIntersections.size();
	m_nY = (int)m_refIntersections.size();

	initAdjustableParameters();
	m_refData.resize(m_nY	, 2);
	for (int i = 0;i < m_nY;++i)
	{
		m_refData(i,0) = m_refIntersections[i].x;
		m_refData(i,1) = m_refIntersections[i].y;
	}
...
bool AffineLineRegistration::intersection(const cvline_polar &l1, const cvline_polar &l2, vector<fPoint>& outPointList, double maxSize/* = 2000*/)
{
	double threshold_dist = 1.0;
	double minAngle = 10.0*3.14159265 / 180.0;
	double delta_angle = fabs(l1.theta - l2.theta);
	if (delta_angle >= 3.14159265/2.0)
	{
		delta_angle -= 3.14159265/2.0;
	}
	if (delta_angle < minAngle)
	{
		return false;
	}

	double rho1 = l1.rho;
	double sin_theta1 = sin(l1.theta);
	double cos_theta1 = cos(l1.theta);
	double rho2 = l2.rho;
	double sin_theta2 = sin(l2.theta);
	double cos_theta2 = cos(l2.theta);
	double m = cos_theta1*sin_theta2 - sin_theta1*cos_theta2;

	double x = (rho1*cos_theta2 - rho2*cos_theta1)/(m + DBL_EPSILON);
	double y = (rho1*sin_theta2 - rho2*sin_theta1)/(m + DBL_EPSILON);
	double minDist;
	double minDist1, minDist2;

	double d11 = sqrt((l1.pt1.x-x)*(l1.pt1.x-x)+(l1.pt1.y-y)*(l1.pt1.y-y));
	double d12 = sqrt((l1.pt2.x-x)*(l1.pt2.x-x)+(l1.pt2.y-y)*(l1.pt2.y-y));
	minDist1 = min(d11, d12);
	
	double d21 = sqrt((l2.pt1.x-x)*(l2.pt1.x-x)+(l2.pt1.y-y)*(l2.pt1.y-y));
	double d22 = sqrt((l2.pt2.x-x)*(l2.pt2.x-x)+(l2.pt2.y-y)*(l2.pt2.y-y));
	minDist2 = min(d21, d22);
	...
	outPointList.push_back(fPoint(x, y));
	return true;
}

bool AffineLineRegistration::intersection_comparison(const _intersection_struct_& node1, const _intersection_struct_& node2)
{
	if (node1.pt.x < node2.pt.x) return true;
	if (node1.pt.x == node2.pt.y) return node1.pt.y < node2.pt.y;

	return false;
}

//bool AffineLineRegistration::sortIntersections(vector<_intersection_struct_>& pts)
//{
//	std::sort(pts.begin(), pts.end(), intersection_comparison);
//	return true;
//}

bool AffineLineRegistration::ratio_forward_match(const vector<double>& ratioList1, const vector<double>& ratioList2)
{
	for (int i = 0;i < (int)ratioList1.size();++i)
	{
		for (int j = 0;j <(int)ratioList2.size();++j)
		{
			if (approximate(ratioList1[i], ratioList2[j]))
			{
				return true;
			}
		}
...
bool AffineLineRegistration::ratio_inverse_match(const vector<double>& ratioList1, const vector<double>& ratioList2)
{
	for (int i = 0;i < (int)ratioList1.size();++i)
	{
		for (int j = 0;j <(int)ratioList2.size();++j)
		{
			if (approximate(ratioList1[i]*ratioList2[j], 1.0))
			{
				return true;
			}
		}
...
bool AffineLineRegistration::pairwise_distance(const vector<LineStruct>& lineList1, const vector<LineStruct>& lineList2, Eigen::MatrixXd& distMatrix)
{
	int n1 = (int)lineList1.size();
	int n2 = (int)lineList2.size();
	distMatrix.resize(n1, n2);
	FILE *pf = fopen("nMatched.txt", "w+");
	for (int i = 0;i < n1;++i)
	{
		int nRatio1 = lineList1[i].intersections.size() - 2;
		for (int j = 0;j < n2;++j)
		{
			int nRatio2 = lineList2[j].intersections.size() - 2;
			int nForward = 0;
			int nInverse = 0;
			for (int ii=1;ii < nRatio1-1;++ii)
			{
				for (int jj = 1;jj < nRatio2-1;++jj)
				{
					if (ratio_forward_match(lineList1[i].intersections[ii].ratios, lineList2[j].intersections[jj].ratios))
					{
						nForward++;
					}
					if (ratio_inverse_match(lineList1[i].intersections[ii].ratios, lineList2[j].intersections[jj].ratios))
					{
						nInverse++;
					}
				}
			}
			int nDivide = max(nRatio1, nRatio2);
			double nMatched = max(nForward, nInverse) / (double)(nRatio1*nRatio2);
			distMatrix(i, j) = nMatched;
			if (nMatched > 0.5)
			{
				cout<<i<<" vs "<<j<<endl;
			}
...
bool AffineLineRegistration::intersection(const cvline_polar &l, const vector<cvline_polar> &lineList, vector<LineStruct>& outLineList, double threshold_dist/* = 2.0*/)
{
	double minAngle = 10.0*3.14159265 / 180.0;
	LineStruct ls;
	ls.line = l;
	...
	{
		cvline_polar l2 = lineList[i];
		double delta_angle = fabs(l.theta - l2.theta);
		if (delta_angle >= 3.14159265/2.0)
		{
			delta_angle -= 3.14159265/2.0;
		}
		...
		double rho1 = l.rho;
		double sin_theta1 = sin(l.theta);
		double cos_theta1 = cos(l.theta);
		double rho2 = l2.rho;
		double sin_theta2 = sin(l2.theta);
		double cos_theta2 = cos(l2.theta);
		double m = cos_theta1*sin_theta2 - sin_theta1*cos_theta2;
		...
		double d11 = sqrt((l.pt1.x-x)*(l.pt1.x-x)+(l.pt1.y-y)*(l.pt1.y-y));
		double d12 = sqrt((l.pt2.x-x)*(l.pt2.x-x)+(l.pt2.y-y)*(l.pt2.y-y));
		minDist1 = min(d11, d12);
		...
		_intersection_struct_ is;
		is.pt = fPoint(x,y);
		is.theta = l2.theta;
		ls.intersections.push_back(is);
	}

	if (ls.intersections.size() <= 2)
	{
		return false;
	}

	//sortIntersections(ls.intersections);
	std::sort(ls.intersections.begin(), ls.intersections.end(), intersection_comparison);
	int nPoint = (int)ls.intersections.size();
	for (int i = 1;i < nPoint-1;++i)
	{
		ls.intersections[i].ratios.clear();
		for (int ii = 0;ii < i;++ii)
		{
			double dist1 = distance_(ls.intersections[i].pt, ls.intersections[ii].pt);
			for (int jj = i+1;jj < nPoint;++jj)
			{
				double dist2 = distance_(ls.intersections[i].pt, ls.intersections[jj].pt);
				ls.intersections[i].ratios.push_back(dist1/(dist2+DBL_EPSILON));
			}
		}
	}
	outLineList.push_back(ls);
	return true;
}
...
void AffineLineRegistration::updateTransLines()
{
	for (int i = 0;i < m_nX;++i)
	{
		fPoint pt = forward(m_srcIntersections[i]);
		m_transData(i,0) = pt.x;
		m_transData(i,1) = pt.y;

	}
	for (int i = 0; i < (int)m_source_lines_polar.size(); i++)
	{
		m_trans_lines_polar[i] = forward(m_source_lines_polar[i]);
	}
}
...
bool AffineLineRegistration::linear_optimization()
{
	...
	//vector<double> b(nX * 2);
	//vector<double> A(nX * 2 * 6);
	////b = candidates.col(6);
	//vector<double> wgt(nX*2);
	//for(int i = 0;i < (int)nX;++i)
	//{
	//	double x = m_srcIntersections[i].x;
	//	double y = m_srcIntersections[i].y;
	//	double xx = m_w(i, 0);
	//	double yy = m_w(i, 1);
	//	A[2*i*6 + 0] = 1.0;
	//	A[2*i*6 + 1] = x;
	...
	//real_1d_array c;
	//lsfitreport rep;
	//fmatrix.setcontent(nX*2, 6, &A[0]);
	//y.setcontent(nX*2, &b[0]);
	//w.setcontent(nX*2, &wgt[0]);
	//lsfitlinearw(y, w, fmatrix, info, c, rep);

	//for(int i=0;i<get_PARAMETER_NUM();++i) m_parameters[i] = c[i];
