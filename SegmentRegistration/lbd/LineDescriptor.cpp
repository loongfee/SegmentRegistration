...
int LineDescriptor::OctaveKeyLines(cv::Mat & image, ScaleLines &keyLines)
{
	...
	for(unsigned int octaveCount = 0; octaveCount<numOfOctave_; octaveCount++){
		...
		cv::GaussianBlur(image, blur, cv::Size(ksize_,ksize_), increaseSigma, 3, cv::BORDER_CONSTANT);
...
//		}
		...
		cv::Size sz((int)(blur.rows/factor), (int)(blur.cols/factor));
		//img.create((int)(blur.rows/factor), (int)(blur.cols/factor), CV_8SC1);
		...
		cv::resize(blur, img, sz, 0, 0, cv::INTER_CUBIC);
		preSigma2 = curSigma2;
  ...
  unsigned int tempID;
	...
	for(unsigned int  lineID = 0;lineID < numOfFinalLine; lineID++){
		...
		if(shouldChange){
			...
			singleLine.sPointInOctaveX = tempValue * e1;
			singleLine.sPointInOctaveY = tempValue * e2;
			singleLine.ePointInOctaveX = tempValue * s1;
			singleLine.ePointInOctaveY = tempValue * s2;
			...
			singleLine.sPointInOctaveX = tempValue * s1;
			singleLine.sPointInOctaveY = tempValue * s2;
			singleLine.ePointInOctaveX = tempValue * e1;
			singleLine.ePointInOctaveY = tempValue * e2;
		...
		if (0 == singleLine.octaveCount)
		{
			LinesVec line_;
			line_.push_back(singleLine);
			keyLines.push_back(line_);
		}
	}
...
int LineDescriptor::ComputeLBD_(ScaleLines &keyLines)
{
	...
	for(short lineIDInScaleVec = 0; lineIDInScaleVec<numOfFinalLine; lineIDInScaleVec++){
		...
		for(short lineIDInSameLine = 0; lineIDInSameLine<sameLineSize; lineIDInSameLine++){
			...
			pdxImg = (short*)edLineVec_[octaveCount]->dxImg_.data;//->data.s;
			pdyImg = (short*)edLineVec_[octaveCount]->dyImg_.data;//->data.s;
