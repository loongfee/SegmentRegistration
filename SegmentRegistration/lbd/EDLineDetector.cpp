...
void EDLineDetector::InitEDLine_()
{
	...
	ATA.resize(2, 2);
	ATV.resize(2);
	tempMatLineFit.resize(2,2);
	tempVecLineFit.resize(2);
	fitMatT.resize(2,minLineLen_);
	fitVec.resize(minLineLen_);
	//ATA.newsize(2,2);
	//ATV.newsize(2);
...
int EDLineDetector::EdgeDrawing(cv::Mat &image, EdgeChains &edgeChains, bool smoothed )
{
	...
	if(!smoothed){//input image hasn't been smoothed.
		cv::GaussianBlur(image, image, cv::Size(ksize_,ksize_), sigma_, 3, cv::BORDER_CONSTANT);
		//CvMat* cvInImage;
	...
	if(gImg_.cols!= imageWidth||gImg_.rows!= imageHeight){
		...
		gImg_.create(imageHeight, imageWidth, CV_32FC1);//.Init(imageWidth,imageHeight,1);
		dirImg_.create(imageHeight, imageWidth, CV_32FC1);
		edgeImage_.create(imageHeight, imageWidth, CV_32FC1);
	...
	short *pdxImg = (short*)(dxImg_.data);//dxImg_->data.s;
	short *pdyImg = (short*)(dyImg_.data);//dyImg_->data.s;
	...
	pdxImg = (short*)dxImg_.data;//->data.s;
	pdyImg = (short*)dyImg_.data;//->data.s;
	pgImg  = gImg_.data;//.GetImageData();
	pdirImg = dirImg_.data;//.GetImageData();
#ifdef DEBUGEdgeDrawing
	timer.Stop();
...
//	}
	...
	edgeImage_=0;//.SetZero();
	unsigned char *pEdgeImg = edgeImage_.data;//GetImageData();
...
int EDLineDetector::EDline(cv::Mat &image, LineChains &lines, bool smoothed)
{
	...
	std::vector<double> lineEquation(2);//[a,b] for lines y=ax+b(horizontal) or x=ay+b(vertical)
	lineEquations_.clear();
	lineEndpoints_.clear();
	lineDirection_.clear();
	unsigned char *pdirImg = dirImg_.data;//.GetImageData();
	...
	for(unsigned int edgeID=0; edgeID<edges.numOfEdges; edgeID++){
		...
		while(offsetInEdgeArrayE > offsetInEdgeArrayS+minLineLen_){//extract line segments from an edge, may find more than one segments
			...
			if(pdirImg[pEdgeYCors[offsetInEdgeArrayS]*imageWidth + pEdgeXCors[offsetInEdgeArrayS]]==Horizontal){//y=ax+b, i.e. ax-y+b=0
				...
				Eigen::Vector3d lineEqu(lineEquation[0]*coef1, -1*coef1, lineEquation[1]*coef1);
				if(LineValidation_(pLineXCors,pLineYCors,pLineSID[numOfLines],offsetInLineArray,lineEqu,direction)){//check the line
					...
					//Eigen::Vector4f lineEndP;//line endpoints
					std::vector<float> lineEndP(4);//line endpoints
				...
				//the line equation coefficients,for line w1x+w2y+w3 =0, we normalize it to make w1^2+w2^2 = 1.
				Eigen::Vector3d lineEqu(1*coef1,-lineEquation[0]*coef1, -lineEquation[1]*coef1);
				if(LineValidation_(pLineXCors,pLineYCors,pLineSID[numOfLines],offsetInLineArray,lineEqu,direction)){//check the line
					//store the line equation coefficients
					lineEquations_.push_back(lineEqu);
					/*At last, compute the line endpoints and store them.
		...
		unsigned int offsetS,	std::vector<double> &lineEquation)
{
	if(lineEquation.size()!=2){
		lineEquation.resize(2);//.newsize(2);
	}
	//int * pMatT;
	//int * pATA;
	double fitError = 0;
	...
	if(pdirImg[yCors[offsetS]*imageWidth + xCors[offsetS]]==Horizontal){
		 ...
		 *    .   [b]  =   .
		 * [xn,1]         [yn]*/
		//pMatT= fitMatT.data;//.GetData();//fitMatT = [x0, x1, ... xn; 1,1,...,1];
		for(int i=0; i<minLineLen_; i++){
			//*(pMatT+minLineLen_) = 1; //the value are not changed;
			fitMatT(0,i)           = xCors[offsetS];
			fitVec[i]            = yCors[offsetS++];
		}
		ATA = fitMatT*fitMatT.transpose();
		//fitMatT.MultiplyWithTransposeOf(fitMatT, ATA);
		ATV = fitMatT * fitVec;
		/* [a,b]^T = Inv(mat^T * mat) * mat^T * vec */
		//pATA = ATA.data;
		coef = 1.0/(double(ATA(0))*double(ATA(3)) - double(ATA(1))*double(ATA(2)));
		//		lineEquation = svd.Invert(ATA) * matT * vec;
		lineEquation[0] = coef *( double(ATA(3))*double(ATV[0])-double(ATA(1))*double(ATV[1]));
		lineEquation[1] = coef *( double(ATA(0))*double(ATV[1])-double(ATA(2))*double(ATV[0]));
		...
		unsigned int offsetE,	std::vector<double> &lineEquation)
{
	...
	if(lineEquation.size()!=2){
		lineEquation.resize(2);
	}
	Eigen::MatrixXi  matT(2,newLength);
	Eigen::VectorXi  vec(newLength);
	//int * pMatT;
	//int * pATA;
	//	double fitError = 0;
	double coef;
	...
	if(pdirImg[yCors[offsetS]*imageWidth + xCors[offsetS] ]==Horizontal){
		 ...
		 *    .    [b]  =   .
		 * [xn',1]         [yn']*/
		//pMatT = matT.data;//.GetData();//matT = [x0', x1', ... xn'; 1,1,...,1]
		//int pos = 0;
		for(int i=0; i<newLength; i++){
			matT(0,i)=xCors[newOffsetS];
			matT(1,i)=1;
			//matT(newLength+pos) = 1;
			//matT(pos++) = xCors[newOffsetS];
			//*(pMatT+newLength) = 1;
			//*(pMatT++)         = xCors[newOffsetS];
			vec[i]             = yCors[newOffsetS++];
		}
		/* [a,b]^T = Inv(ATA + mat^T * mat) * (ATV + mat^T * vec) */
		tempMatLineFit = matT * matT.transpose();
	...
	if(pdirImg[yCors[offsetS]*imageWidth + xCors[offsetS] ]==Vertical){
		 ...
		 *   .    [b]  =   .
		 * [yn',1]         [xn']*/
		//pMatT = matT.data;//.GetData();//matT = [y0', y1', ... yn'; 1,1,...,1]
		//int pos = 0;
		for(int i=0; i<newLength; i++){
			matT(0,i)=yCors[newOffsetS];
			matT(1,i)=1;
			//matT(pos+newLength) = 1;
			//matT(pos++) = yCors[newOffsetS];
			//*(pMatT+newLength) = 1;
...
bool EDLineDetector::LineValidation_(  unsigned int *xCors,   unsigned int *yCors,
		...
		Eigen::Vector3d &lineEquation, float &direction)
{
	if(bValidate_){
		...
		short *pdxImg = (short*)dxImg_.data;//->data.s;
		short *pdyImg = (short*)dyImg_.data;//->data.s;
		double dx, dy;
		cv::Vector<double> pointDirection(n);
		double *pPointDirection = (double*)&pointDirection[0];//.GetData();
		int index;
		for(int i=0; i<n; i++){
...
int EDLineDetector::EDline(cv::Mat &image, bool smoothed)
{
	...
	unsigned char *pgImg  = gImgWO_.data;//->data.ptr;
