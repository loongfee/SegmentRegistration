...
public:
	...
	std::vector<Eigen::Vector3d > lineEquations_;
	//store the line endpoints, [x1,y1,x2,y2]
	//std::vector<Eigen::Vector4f> lineEndpoints_;
...
private:
  ...
  bool bValidate_;//flag to decide whether line will be validated
	...
	double logNT_;
	Eigen::MatrixXi ATA;	 //the previous matrix of A^T * A;
	Eigen::VectorXi ATV;    //the previous vector of A^T * V;
	Eigen::MatrixXi fitMatT;	 //the matrix used in line fit function;
	Eigen::VectorXi fitVec;    //the vector used in line fit function;
	Eigen::MatrixXi tempMatLineFit;	 //the matrix used in line fit function;
	Eigen::VectorXi tempVecLineFit;    //the vector used in line fit function;
