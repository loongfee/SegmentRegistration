...
void PairwiseLineMatching::BuildAdjacencyMatrix_(ScaleLines &linesInLeft,ScaleLines &linesInRight)
{
	...
	Eigen::MatrixXd desDisMat(numLineLeft,numLineRight);//store the descriptor distance of lines in left and right images.
...
//	desDisMat.Save("DescriptorDis.txt");
	 ...
	 */
	Eigen::Matrix<unsigned int, -1,-1,0,-1,-1> bComputedLeft(numLineLeft,numLineLeft);//flag to show whether the ith pair of left image has already been computed.
	bComputedLeft.setZero();//.SetZero();
	Eigen::MatrixXd intersecRatioLeft(numLineLeft,numLineLeft);//the ratio of intersection point and the line in the left pair
	Eigen::MatrixXd projRatioLeft(numLineLeft,numLineLeft);//the point to line distance divided by the projected length of line in the left pair.

	Eigen::Matrix<unsigned int, -1,-1,0,-1,-1> bComputedRight(numLineRight,numLineRight);//flag to show whether the ith pair of right image has already been computed.
	bComputedRight.setZero();//.SetZero();
	Eigen::MatrixXd       intersecRatioRight(numLineRight,numLineRight);//the ratio of intersection point and the line in the right pair
	Eigen::MatrixXd       projRatioRight(numLineRight,numLineRight);//the point to line distance divided by the projected length of line in the right pair.
		...
		std::vector<unsigned int > &matchResult)
{
  ...
  Eigen::MatrixXd mat(linesInLeft.size(),linesInRight.size());
  mat.setZero();//.SetZero();
