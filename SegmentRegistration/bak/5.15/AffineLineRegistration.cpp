...
{
	double minAngle = 85.0*3.14159265 / 180.0;
	...
	double minDist = (maxSize/55.0);
	minDist *= minDist;
	for (int i = 0; i < (int)outPointList.size(); i++)
	{
		double dist = (x-outPointList[i].x)*(x-outPointList[i].x) + (y-outPointList[i].y)*(y-outPointList[i].y);
		if (dist < minDist)
		{
			return false;
		}
...
bool AffineLineRegistration::linear_optimization()
{
	...
	{		
		...
		//coeff[0] = 1.0;
		//coeff[1] = m_srcIntersections[i].x;
		//coeff[2] = m_srcIntersections[i].y;
		//coeff[3] = 0.0;
		//coeff[4] = 0.0;
		//coeff[5] = 0.0;
		...
		//{        
		//	b[p1] += coeff[p1] * m_refIntersections[i].x * weight;
		...
		//coeff[2] = 0.0;
		//coeff[3] = 1.0;
		//coeff[4] = m_srcIntersections[i].x;
		//coeff[5] = m_srcIntersections[i].y;
		//for(int p1=0;p1<6;++p1)
		//{        
		//	b[p1] += coeff[p1] * m_refIntersections[i].y * weight;
		//	for(int p2=0;p2<6;++p2)
		//	{
		//		A(p1,p2) += coeff[p1] * coeff[p2] * weight;
		//	}
